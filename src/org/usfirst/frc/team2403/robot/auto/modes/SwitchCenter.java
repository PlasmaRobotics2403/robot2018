...
public class SwitchCenter extends AutoMode{ 
	...
	public SwitchCenter(DriveTrain drive, Intake intake, Elevator elevator) {
		this.drive = drive;
		this.intake = intake;
	...
	protected void routine() throws AutoModeEndedException {		
		if(gameData.charAt(0) == 'R') {
			...
			runAction(new Clamp(intake, true));
			runAction(new FollowTrajectory("SwitchCenterLeft1", drive));
			runAction(new IntakeCube(.7, false, intake));
			runActionsParallel(new PivotAngle(Constants.PIVOT_POSITION_BOTTOM, elevator), new FollowTrajectory("SwitchCenterLeft2", drive));
			runAction(new IntakeCube(.8, true, intake));
			runAction(new Clamp(intake, false));
			runAction(new FollowTrajectory("SwitchCenterLeft3", drive));
			runAction(new Clamp(intake, true));
			runAction(new IntakeCube(0, true, intake));
			runActionsParallel(new PivotAngle(Constants.PIVOT_POSITION_SWITCH, elevator), new FollowTrajectory("SwitchCenterLeft4", drive));
			runAction(new FollowTrajectory("SwitchCenterLeft5", drive));
			runAction(new IntakeCube(.6, false, intake));
			runAction(new Wait(1));
			runAction(new IntakeCube(0, true, intake));
		}
